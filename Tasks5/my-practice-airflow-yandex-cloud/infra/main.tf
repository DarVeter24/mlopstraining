...
resource "null_resource" "update_env" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      # Получаем AIRFLOW_HOST с отладкой
      echo "Getting cluster info..."
      CLUSTER_INFO=$(yc managed-airflow cluster get ${module.airflow-cluster.cluster_id} --format json)
      echo "Raw cluster info: $CLUSTER_INFO"
      
      # Извлекаем URL и убираем протокол
      AIRFLOW_HOST=$(echo "$CLUSTER_INFO" | jq -r '.endpoints.webserver // "not-found"' | sed 's|^https\?://||')
      echo "Extracted AIRFLOW_HOST: $AIRFLOW_HOST"
      
      # Если не найден, попробуем другие варианты
      if [ "$AIRFLOW_HOST" = "not-found" ] || [ -z "$AIRFLOW_HOST" ]; then
        echo "Webserver endpoint not found, trying alternative..."
        AIRFLOW_HOST=$(echo "$CLUSTER_INFO" | jq -r '.webserver_url // "manual-check-needed"' | sed 's|^https\?://||')
      fi
      
      # Добавляем или обновляем AIRFLOW_HOST
      if grep -q "^AIRFLOW_HOST=" ../.env; then
        sed -i "s|^AIRFLOW_HOST=.*|AIRFLOW_HOST=$AIRFLOW_HOST|" ../.env
      else
        echo "AIRFLOW_HOST=$AIRFLOW_HOST" >> ../.env
      fi
      
      echo "Final AIRFLOW_HOST: $AIRFLOW_HOST"
      echo "=== Env update completed ==="
  ...
  depends_on = [
    module.iam,
    module.storage,
    module.airflow-cluster  # добавьте эту зависимость
  ]
}
