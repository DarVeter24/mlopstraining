...
![Grafana Alert Rules](images/alert-rules.png)

На скриншоте показана настройка правил алертинга в Grafana:
- **AdminNotification_MaxScaleHighCPU**: правило для мониторинга критической ситуации когда достигнуто максимальное количество подов (6) и CPU нагрузка превышает 80% в течение 5 минут
- **Статус**: правило активно и готово к срабатыванию при достижении пороговых значений
- **Evaluation**: проверка каждую минуту с задержкой срабатывания 5 минут для избежания ложных срабатываний

**Grafana Contact Points Configuration:**

![Grafana Contact Points](images/cntact-points.png)

На скриншоте показана настройка точек контакта для уведомлений:
- **telegram-admin-bot**: настроенный Telegram бот для отправки критических уведомлений системному администратору
- **Integration**: успешная интеграция с Telegram API для доставки алертов в реальном времени
- **Configuration**: корректно настроенные параметры Bot Token и Chat ID для надежной доставки уведомлений
...
**🔧 Bare Metal Implementation:** Система Apache Airflow развернута в Kubernetes кластере на виртуальных машинах Proxmox VE с использованием ArgoCD и Helm чарта. Подробная информация о развертывании Airflow представлена в [README.md из Tasks6](../Tasks6/README.md), где показан полный процесс настройки и конфигурации системы оркестрации ML пайплайнов.

**Конфигурационные файлы для Airflow:**
- ArgoCD манифест для развертывания Airflow в Kubernetes
- Dockerfile для кастомного образа с необходимыми зависимостями
- Requirements.txt с Python библиотеками для ML задач

**Интеграция с MLflow:**
- Автоматическое логирование метрик обучения модели
- Сохранение артефактов в MinIO S3-совместимое хранилище
- Периодическое переобучение модели по расписанию


**Git Sync Configuration для загрузки DAG'ов:**

Airflow настроен на автоматическую загрузку DAG'ов из GitHub репозитория через Git Sync механизм. Конфигурация в [`argocd/airflow/airflow-helm.yaml`](./argocd/airflow/airflow-helm.yaml):

Скопирован и адаптирован DAG для Apache Airflow из Tasks6, включающий обучение модели с использованием PySpark на данных о мошеннических транзакциях.

**Основные файлы:**
- [DAG файл simple_training_pipeline.py](./dag/simple_training_pipeline.py) - основной пайплайн обучения модели (скопирован из Tasks6)

**Airflow DAG для переобучения модели:**

![Simple Fraud Detection Training](./images/simple_fraud_detection_training.png)

На скриншоте показан успешный запуск DAG `simple_fraud_detection_training` в веб-интерфейсе Airflow. Видно, что задача `train_model` выполнена успешно (зеленый статус) с продолжительностью 00:02:47, что подтверждает корректную работу пайплайна переобучения модели обнаружения мошенничества.

**Детальный вид выполнения DAG:**

![Simple Fraud Detection Training Very](./images/simple_fraud_detection_training_very.png)

Детальный вид показывает полную информацию о выполнении DAG:
- **Статус**: ✅ Success (успешное завершение)
- **Время выполнения**: 2025-09-13, 20:24:30 - 20:27:17
- **Продолжительность**: 00:02:47
- **Версия DAG**: v1
- **Operator**: PythonOperator для выполнения обучения модели

**MLFlow Experiment - Training Run:**

![Training 20250913 1725](./images/training_20250913_1725.png)

Скриншот MLFlow показывает детали эксперимента обучения модели:
- **Run Name**: training_20250913_1725
- **Experiment ID**: 6
- **Status**: 🔄 Running (в процессе выполнения)
- **Created**: 2025-09-13, 20:25:52 PM
- **Duration**: отслеживание времени выполнения
- **Интеграция**: полная интеграция Airflow с MLFlow для трекинга экспериментов

**MLFlow Artifacts - Model Storage:**

![Training Artifacts](./images/training_20250913_1725_artifacts.png)

Скриншот показывает артефакты сохраненной модели в MLFlow:
- **fraud_detection_model**: основная папка модели
- **sparkml**: Spark ML Pipeline модель
- **MLmodel**: метаданные модели
- **conda.yaml**: зависимости окружения
- **python_env.yaml**: Python окружение
- **requirements.txt**: Python зависимости
- **Path**: s3://mlflow-artifacts/... - сохранение в MinIO S3-совместимом хранилище


**Автоматизированная сборка и деплой модели через CI/CD пайплайн:**

**GitHub Actions CI/CD Pipeline** ([`tasks10-ci.yml`](./.github/workflows/tasks10-ci.yml)) обеспечивает полную автоматизацию процесса развертывания:

**🔄 Этапы CI/CD процесса:**

1. **Тестирование кода** (`test` job):
   - Матричное тестирование на Python 3.11, 3.12, 3.13
   - Кэширование pip зависимостей для ускорения сборки
   - Проверка базовых импортов и структуры проекта
   - Создание тестового окружения из `env-example`

2. **Линтинг и валидация** (`lint` job):
   - Проверка синтаксиса Python кода
   - Валидация структуры файлов проекта
   - Базовая проверка качества кода

3. **Docker тестирование** (`docker-test` job):
   - Сборка Docker образа для тестирования
   - Использование Docker Buildx с кэшированием
   - Проверка размера и слоев образа
   - Валидация корректности сборки

4. **Сборка и публикация** (`build-and-push` job):
   - **Условие**: только для `main` ветки при push
   - Авторизация в GitHub Container Registry (ghcr.io)
   - Генерация метаданных и тегов образа
   - Публикация в `ghcr.io/username/tasks10-ml-service-api`
   - Создание артефактов деплоя с информацией о сборке
