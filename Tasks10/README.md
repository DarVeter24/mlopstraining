...
🎯 **Predictions per Minute**: График показывает рост активности модели с 0 до 0.5 запросов в секунду в период с 16:50 до 17:40. Видно постепенное увеличение нагрузки на ML API, что соответствует тестированию системы под нагрузкой.

⏱️ **Average Prediction Time**: Gauge показывает среднее время ответа модели **2.90 ms**, что является отличным показателем производительности. Зеленый цвет индикатора подтверждает, что время ответа находится в пределах нормы (< 1000ms).

Данные метрики подтверждают корректную работу ML API и позволяют отслеживать производительность модели машинного обучения в режиме реального времени.
