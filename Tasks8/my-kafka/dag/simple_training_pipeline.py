...
Description: Обучение модели мошенничества с PySpark, MLflow и автоматической регистрацией
Версия: 3.0 (обновлено с MLflow 2.15.1, Airflow Variables и регистрацией champion)
...
✅ Установка stage "Production" для совместимости с Tasks8 (MLflow 2.9+)
✅ Полная интеграция с spark_stream_inference.py
...
def train_model_python():
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        # Логирование и регистрация модели с stage "champion"
        try:
            # 1. Логируем модель в текущий run
            mlflow.spark.log_model(model, "fraud_detection_model")
            print("✅ Модель залогирована в MLflow успешно")
            
            # 2. Регистрируем модель в Model Registry
            model_uri = f"runs:/{mlflow.active_run().info.run_id}/fraud_detection_model"
            print(f"📋 URI модели для регистрации: {model_uri}")
            ...
            try:
                ...
                print(f"✅ Модель зарегистрирована в Model Registry: версия {model_details.version}")
                
                # 3. Устанавливаем stage "champion"
                client = mlflow.tracking.MlflowClient()
                
                # Сначала переводим все существующие версии в "Archived"
                try:
                    ...
                    for version in existing_versions:
                        if version.current_stage == "Production":
                            ...
                            print(f"🔄 Версия {version.version} переведена в Archived")
                except Exception as e:
                    print(f"ℹ️ Нет существующих production версий или ошибка: {e}")
                
                # Устанавливаем новую версию как "Production" (современный MLflow)
                client.transition_model_version_stage(
                    name="fraud_detection_model",
                ...
                print(f"🏆 Модель {model_details.version} успешно установлена как PRODUCTION!")
                print(f"🎯 Теперь Tasks8 сможет загрузить модель: fraud_detection_model@Production")
                
            except Exception as register_error:
                print(f"❌ Ошибка регистрации модели: {register_error}")
                mlflow.log_param("model_registration_error", str(register_error))
                
        except Exception as model_log_error:
            print(f"❌ КРИТИЧЕСКАЯ ОШИБКА логирования модели: {model_log_error}")
            ...
            try:
                ...
                print(f"❌ Ошибка сохранения как артефакт: {artifact_error}")
...
with DAG(
    ...
    description="Обучение модели мошенничества с PySpark, MLflow и автоматической регистрацией как champion",
